...
<body>
    ...
    <div class="content">
        <form id="form" method="POST" action="index.php">
            ...
            <input type="text" name="phone" placeholder="88005553535" value="<?= htmlspecialchars($data['phone'] ?? '', ENT_QUOTES, 'UTF-8') ?>" class="<?= isset($errors['phone']) ? 'error-field' : '' ?>">
            <br>

            <label for="email">E-mail:</label>
            <input type="text" name="email" placeholder="you@example.com" value="<?= htmlspecialchars($data['email'] ?? '', ENT_QUOTES, 'UTF-8') ?>" class="<?= isset($errors['email']) ? 'error-field' : '' ?>">
            <br>

            <label for="birthday">Дата рождения:</label>
            <input type="date" name="birthday" value="<?= htmlspecialchars($data['birthday'] ?? '', ENT_QUOTES, 'UTF-8') ?>" class="<?= isset($errors['birthday']) ? 'error-field' : '' ?>">
            <br>
