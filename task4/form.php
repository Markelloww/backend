...
<body>
    ...
    <div class="content">
        <form id="form" method="POST" action="index.php">
            ...
            <input type="text" name="name" placeholder="Введите ваши ФИО" type="text" value="<?= htmlspecialchars($data['name'] ?? '') ?>" class="<?= isset($errors['name']) ? 'error-field' : '' ?>">
            <br>

            <label for="phone">Телефон:</label>
            <input type="text" name="phone" placeholder="88005553535" type="tel" value="<?= htmlspecialchars($data['phone'] ?? '') ?>" class="<?= isset($errors['phone']) ? 'error-field' : '' ?>">
            <br>

            <label for="email">E-mail:</label>
            <input type="text" name="email" placeholder="you@example.com" type="email" value="<?= htmlspecialchars($data['email'] ?? '') ?>" class="<?= isset($errors['email']) ? 'error-field' : '' ?>">
            <br>

            <label for="birthday">Дата рождения:</label>
            <input type="date" name="birthday" type="date" value="<?= htmlspecialchars($data['birthday'] ?? '') ?>" class="<?= isset($errors['birthday']) ? 'error-field' : '' ?>">
            <br>
